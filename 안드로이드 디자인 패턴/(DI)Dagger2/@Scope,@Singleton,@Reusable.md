...
* 아마도 추측으로는 싱글톤처럼 역할은 똑같지만 일반적인 싱글톤이 어플리케이션 전체에 걸쳐서 살아있다면 CustomScope로 설정할경우 해당 CustomScope로 설정한 컴포넌트가 살아있는 동안은 싱글톤 유지 되고 만약에 Destory 될경우는 비록 싱글톤이라도 사라진다. 하지만 언제 Component가 Destory가 되는지는 잘모르겠음..[
