...
---
### 안드로이드에서 Application() 을 통해 접근하면 될것을 왜 Dagger를 사용하는것인지?
* 많은 상용구 코드(보일러 코드)와 추가 작업의 단점
* Dagger와 같이 잘 고안된 라이브러리에 비해 **오류가 발생하기 쉽고 효율적이거나 견고하지 않을 수 있다.** 
* 새로운 개발자를 온 보딩하기가 더 불리하다.
---
### 흐름
* Component에 어떤 객체를 주입하면 해당 객체에서 필요한것(@Inject=외부로부터 주입받을것)을 모듈에서 제공(@Provides) 된것들중 같은 타입인것으로 이어준다. 이를 Dagger가 처리해준다. 그리고 Component는 이들을 바인딩 해주느 역할을 해준다.
* 의존성을 요청받으면 Subcomponent, Component, Inject 생성자 순으로 검색하여 주입
---
### Dagger2 에서 컴포넌트와 모듈
* ```java
